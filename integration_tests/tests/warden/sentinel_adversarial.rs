...
"#;
    ...
    let report1 = engine.sanitize_prompt(bytes::Bytes::from(prompt1.to_string()),  None).await.unwrap();
    ...
    let report2 = engine.sanitize_prompt(bytes::Bytes::from(prompt2.to_string()),  None).await.unwrap();
...
#[tokio::test]
async fn test_semantic_cache_poisoning() {
...
"#;
    ...
    let report1 = engine2.sanitize_prompt(bytes::Bytes::from("Hello Alice".to_string()),  Some(&session)).await.unwrap();
    ...
    let report2 = engine2.sanitize_prompt(bytes::Bytes::from(prompt2.to_string()),  Some(&session)).await.unwrap();
