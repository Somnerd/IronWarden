...
async fn test_semantic_cache_homoglyph_resilience() {
    ...
    let report = engine.sanitize_prompt(bytes::Bytes::from("The name is Al\u{0456}ce Smith.".to_string()),  Some(&session)).await.unwrap();
...
async fn test_shadow_ner_promotion_without_ai() {
    ...
    let report = engine.sanitize_prompt(bytes::Bytes::from("\u{0391}\u{03BB}\u{03AD}\u{03BE}\u{03B1}\u{03BD}\u{03B4}\u{03C1}\u{03BF}\u{03C2} is here.".to_string()),  None).await.unwrap();
...
async fn test_shadow_ner_homoglyph_resilience() {
    ...
    let pepper = SecretVec::new(vec![0u8; 32]);
    let engine = WardenEngine::new(vec![], vec![], vec![], None, 0.85, &pepper).unwrap();
    let report = engine.sanitize_prompt(bytes::Bytes::from("Hello Al\u{0456}ce Smith.".to_string()),  None).await.unwrap();
