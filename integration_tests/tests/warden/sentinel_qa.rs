...
#[tokio::test]
    ...
    let report = engine.sanitize_prompt("The name is Al\u{0456}ce Smith.", Some(&session)).await.unwrap();
...
#[tokio::test]
    ...
    let pepper = SecretVec::new(vec![0u8; 32]);
    let engine = WardenEngine::new(vec![], vec![], vec![], None, 0.85, &pepper).unwrap();
    let report = engine.sanitize_prompt("Hello Al\u{0456}ce Smith.", None).await.unwrap();
