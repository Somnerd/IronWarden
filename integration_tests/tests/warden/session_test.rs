...
"#;
    ...
    let report1 = shield.sanitize_prompt(bytes::Bytes::from(input1.to_string()),  Some(&mut session)).await.unwrap();
    ...
    let report2 = shield.sanitize_prompt(bytes::Bytes::from(input2.to_string()),  Some(&mut session)).await.unwrap();
    ...
    let input3 = "Bob left.";
    let report3 = shield.sanitize_prompt(bytes::Bytes::from(input3.to_string()),  Some(&mut session)).await.unwrap();
