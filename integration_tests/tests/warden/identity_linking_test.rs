...
mod tests {
    ...
    async fn test_identity_linking_john_and_alice_smith() {
        ...
        let report1 = engine
            .sanitize_prompt("Hello John Smith.", Some(&session))
            .await
            .unwrap();
        let john_token = report1
            .token_map
            .iter()
            .find(|(_, v)| *v == "John Smith")
        ...
        let report2 = engine2
            .sanitize_prompt("Hello Alice Smith.", Some(&session))
            .await
            .unwrap();
        let alice_token = report2
            .token_map
            .iter()
            .find(|(_, v)| *v == "Alice Smith")
            .map(|(k, _): (&String, &String)| k.clone())
            .unwrap();

        assert_ne!(
            john_token, alice_token,
            "John and Alice should have distinct tokens"
        );
        ...
        let report3 = engine3
            .sanitize_prompt("Smith is here.", Some(&session))
            .await
            .unwrap();
        let smith_token = report3
            .token_map
            .iter()
            .find(|(_, v)| *v == "Smith")
            .map(|(k, _): (&String, &String)| k.clone())
