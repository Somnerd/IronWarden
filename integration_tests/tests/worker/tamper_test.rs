...
async fn test_gap_02_boot_handshake_tamper_detection() {
    ...
    // 1. Initialize a clean auditor to create the DB and anchor
    {
        ...
        let auditor = AsyncAuditor::spawn(db_path.to_str().unwrap(), pepper, None)
            .await
            .unwrap();
        ...
        auditor
            .log_report(report, "test".to_string(), "user_1".into())
            .await
            .unwrap();
    ...
    conn.execute(
        "UPDATE audit_reports SET integrity_hash = 'TAMPERED' WHERE id = 1",
        [],
    )
    .unwrap();
    ...
    match result {
        ...
        Err(e) => {
            ...
            assert!(
                err_msg.contains("DB Init Failed"),
                "Error should indicate DB initialization failure"
            );
        }
    }
}
