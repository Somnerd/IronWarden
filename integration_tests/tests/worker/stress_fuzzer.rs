...
async fn test_time_travel_purge_and_hmac_integrity() {
    ...
    // 3. Time-Travel: Backdate the first log by 31 days
    {
        ...
        conn.execute("UPDATE ephemeral_raw_logs SET timestamp = ?1 WHERE id = 1", [&cutoff_str]).unwrap();
    }
    ...
    // (Simulates the 30-day automated worker purge)
    {
        ...
        let cutoff_str = cutoff.format("%Y-%m-%d %H:%M:%S").to_string();
        conn.execute("DELETE FROM ephemeral_raw_logs WHERE timestamp < ?1", [&cutoff_str]).unwrap();
    }
    ...
    assert!(stdout.contains("Verified Intact Logs: 1"), "Expected 1 verified log");
    assert!(stdout.contains("Verified Archived Logs: 1"), "Expected 1 archived log");
    assert!(stdout.contains("STATUS: CHAIN INTACT"), "Expected chain intact status");

    // 6. Adversarial Tamper Test
    // A malicious actor alters the blocked status in the database
    {
        let conn = Connection::open(db_path).unwrap();
        conn.execute("UPDATE audit_reports SET is_blocked = 0 WHERE id = 2", []).unwrap();
    }
    ...
    assert!(stdout_tampered.contains("TAMPER DETECTED at Log ID 2"), "Expected tamper detection on ID 2");
