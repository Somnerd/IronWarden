...
async fn test_v14_leak_prevention_enforced() {
    ...
    let librarian = Arc::new(
        LocalLibrarian::new(knowledge_path.to_str().unwrap())
            .await
            .unwrap(),
    );
    librarian
        .add_document(
            "The secret project is code-named Project Icarus.",
            "test_user",
        )
        .await
        .unwrap();
    ...
    let engine = WardenEngine::new(
        vec![(
            "rule1".into(),
            "Icarus".into(),
            iw_core::EnforcementAction::Redact,
            iw_core::PiiCategory::InternalAsset,
        )],
        vec![],
        vec![],
        None,
    ...
    let result = queue
        .get_result(&job_id, "test_user", true)
        .await
        .unwrap()
        .expect("Job should be complete");
    ...
    assert!(
        ...
        "V-14 Enforcement: Search should NOT have found the raw context using a sanitized query"
    );

    // 2. The result MUST be scrubbed (Double-check)
    assert!(
        !result.contains("Icarus"),
        "Raw PII 'Icarus' must NOT be in the final result"
    );
}
