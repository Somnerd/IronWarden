...
async fn test_v19_session_swap_integrity() {
    ...
    ctx_a.identities.insert("alice".to_string(), "[TOKEN_A]".to_string());
    
    manager.save_session(user_a, &ctx_a).await.expect("Failed to save session A");
    ...
    let session_data: Vec<u8> = conn.query_row("SELECT session_data FROM sessions WHERE username = 'user_a'", [], |r| r.get(0)).unwrap();
    conn.execute("INSERT INTO sessions (username, session_data) VALUES ('user_b', ?1)", [session_data]).unwrap();
