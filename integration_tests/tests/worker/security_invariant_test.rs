...
async fn test_hard_stop_monitor_anchor_missing() {
    ...
    assert!(
        health_res.is_err(),
        "Storage health should be ERR after anchor removal, but got OK"
    );
    assert!(health_res
        .unwrap_err()
        .to_string()
        .contains("Auditor Hard-Stop"));
}
...
async fn test_v19_session_isolation_aad() {
    ...
    let manager =
        worker::LocalSessionManager::new(db_path.to_str().unwrap().to_string(), &pepper).unwrap();
    ...
    ctx_a
        .pii_to_token
        .insert("key1".to_string(), "[TOKEN_1]".to_string());
    ...
    assert!(res_b
        .unwrap_err()
        .to_string()
        .contains("Decryption failed (Integrity Mismatch or Incorrect AAD)"));
}
