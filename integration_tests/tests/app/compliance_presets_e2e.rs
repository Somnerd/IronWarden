...
async fn test_hipaa_compliance_preset_redaction() {
    ...
    let engine = config.compile_engine(&pepper)
    ...
    let report = engine.sanitize_prompt(medical_prompt, None).await
        .expect("Sanitization must succeed");

    assert!(!report.is_blocked);
    assert!(!report.sanitized_text.contains("000-12-3456"), "SSN must be redacted");
    assert!(report.sanitized_text.contains('['), "Placeholder token must be present");

    // Verify roundtrip restoration
    let restored = engine.restore_prompt(&report.sanitized_text, &report.token_map)
        .expect("Restoration must succeed");
    assert!(restored.contains("000-12-3456"), "Restoration must recover original SSN");
}
...
async fn test_gdpr_compliance_preset_redaction() {
    ...
    let report = engine.sanitize_prompt(citizen_prompt, None).await
        .expect("Sanitization must succeed");

    assert!(!report.is_blocked);
    assert!(!report.sanitized_text.contains("citizen.europe@domain.eu"), "Email must be redacted");
    
    let restored = engine.restore_prompt(&report.sanitized_text, &report.token_map)
        .expect("Restoration must succeed");
    assert!(restored.contains("citizen.europe@domain.eu"), "Restoration must recover email");
}
...
async fn test_pci_dss_compliance_preset_redaction() {
    ...
    let payment_prompt = "Process transaction for card 4532-1234-5678-9012 with CVV 789 and exp 12/28.";
    let report = engine.sanitize_prompt(payment_prompt, None).await
        .expect("Sanitization must succeed");

    assert!(!report.is_blocked);
    assert!(!report.sanitized_text.contains("4532-1234-5678-9012"), "Payment card must be redacted");

    let restored = engine.restore_prompt(&report.sanitized_text, &report.token_map)
        .expect("Restoration must succeed");
    assert!(restored.contains("4532-1234-5678-9012"), "Restoration must recover card number");
}
