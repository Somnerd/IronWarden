...
mod tests {
    ...
    async fn test_searchboost_cross_user_isolation_v19() {
        ...
        let job_id = queue.enqueue("query".into(), HashMap::new(), "th1".into(), "userA".into()).await.unwrap();
        ...
        let encrypted_result = iw_core::AadCipher::encrypt(b"secret result", "userA", pepper_clone.expose_secret(), b"warden-v1-queue-encryption").unwrap();
        
        let conn = pool.get().unwrap();
        conn.execute("UPDATE search_jobs SET result = ?1, status = 'complete' WHERE id = ?2", (&encrypted_result, &job_id)).unwrap();
        ...
        assert!(res_b.is_err(), "Cross-user data leakage detected in SearchBoost get_result!");
        assert!(res_b.unwrap_err().to_string().contains("permission to access"));
    ...
    async fn test_searchboost_fifo_ordering() {
        ...
        let job1 = queue.enqueue("query1".into(), HashMap::new(), "th1".into(), "userA".into()).await.unwrap();
        let job2 = queue.enqueue("query2".into(), HashMap::new(), "th1".into(), "userA".into()).await.unwrap();
        let job3 = queue.enqueue("query3".into(), HashMap::new(), "th1".into(), "userA".into()).await.unwrap();
    ...
    async fn test_searchboost_graceful_shutdown() {
        ...
        for i in 0..100 {
            queue.enqueue(format!("query{}", i), HashMap::new(), "th1".into(), "userA".into()).await.unwrap();
        }
        ...
        let count: i64 = conn.query_row("SELECT COUNT(*) FROM search_jobs", [], |r| r.get(0)).unwrap();
    ...
    async fn test_searchboost_redis_ha_fallback() {
        ...
        let job = queue.enqueue("query".into(), HashMap::new(), "th1".into(), "userA".into()).await.unwrap();
        
        tokio::time::sleep(Duration::from_millis(200)).await;

        let conn = queue.pool.get().unwrap();
        let count: i64 = conn.query_row("SELECT COUNT(*) FROM search_jobs WHERE id = ?1", [&job], |r| r.get(0)).unwrap();
        
        assert_eq!(count, 1, "Failed to fallback to SQLite when Redis is unreachable");
