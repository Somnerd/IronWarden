...
#[test]
fn test_semantic_cache_homoglyph_resilience() {
    ...
    let report = engine.sanitize_prompt("The name is Al\u{0456}ce Smith.", Some(&session)).unwrap();
    
    assert_eq!(report.redactions.len(), 1, "Homoglyph should hit the ASCII-keyed cache");
    assert_eq!(report.redactions[0].rule_id, "ai_cache_PERSON");
}

#[test]
fn test_shadow_ner_promotion_without_ai() {
    // Engine without AI
    let pepper = SecretVec::new(vec![0u8; 32]);
    let engine = WardenEngine::new(vec![], vec![], vec![], None, 0.85, &pepper).unwrap();
    ...
    let report = engine.sanitize_prompt("\u{0391}\u{03BB}\u{03AD}\u{03BE}\u{03B1}\u{03BD}\u{03B4}\u{03C1}\u{03BF}\u{03C2} is here.", None).unwrap();
    
    assert_eq!(report.redactions.len(), 1, "Greek name should be promoted without AI");
    assert!(report.redactions[0].rule_id.contains("heuristic_promotion_POTENTIAL_GREEK_NAME"));
}

#[test]
fn test_shadow_ner_homoglyph_resilience() {
    ...
    let pepper = SecretVec::new(vec![0u8; 32]);
    let engine = WardenEngine::new(vec![], vec![], vec![], None, 0.85, &pepper).unwrap();
    let report = engine.sanitize_prompt("Hello Al\u{0456}ce Smith.", None).unwrap();
    
    // Without AI and without cache, it should be promoted to a redaction because it's an IndividualName
    assert_eq!(report.redactions.len(), 1, "Shadow NER should catch homoglyph input and promote it");
    assert_eq!(report.redactions[0].rule_id, "heuristic_promotion_POTENTIAL_GLOBAL_NAME");
}
