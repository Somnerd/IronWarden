...
## 2026-06-22 - [Insecure Data Isolation in LocalLibrarian]
**Vulnerability:** LocalLibrarian manually filtered user documents from a stream after fetching a hardcoded `limit(1000)` records from LanceDB. In a production setting, this causes data starvation/DoS where users with a large document footprint prevent other users from retrieving their own isolated context.
**Learning:** LanceDB provides SQL-like filtering on the Vector Store. User-level isolation must be strictly pushed down to the underlying database engine using the `.only_if` method rather than handling iteration inside the application's memory space. To mitigate SQL injection during `only_if` construction, manual escaping (`replace("'", "''")`) is required in LanceDB's DataFusion parser format.
**Prevention:** Always rely on database-level constraints for multi-tenant data access controls instead of fetching and manually partitioning in application code.
